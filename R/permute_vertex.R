...
#'   \code{\link{group.graph.diffs}}); either 't.test', 'wilcox.test', or 'lm'
...
#' \item{p.max}{The proportion of the number of times the maximum statistic of
#'   the permuted groups was greater than the observed maximum statistic}
#' \item{thresh}{The \eqn{1 - \alpha} \%ile maximum statistic of all permuted
#'   values}
#'
#' @seealso \code{\link{group.graph.diffs}}
...
permute.vertex <- function(g1, g2, measure, test=c('t.test', 'wilcox.test', 'lm'),
                           alpha=0.05, N=1e3, ...) {
  combined <- c(g1, g2)
  g.diffs <- group.graph.diffs(g1, g2, measure, test=test, ...)
